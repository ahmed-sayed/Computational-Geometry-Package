...
namespace CGUtilities.DataStructures
{
    ...
    public class OrderedSet<T> : CollectionBase<T>, ICollection<T>, ICloneable
    {
        ...
        /// <summary>
        /// Get the right and left nodes to the given item
        /// </summary>
        /// <param name="NewItem">Item to get its left and right nodes</param>
        /// <returns>KeyValuepair where the key is the right node and value is the left node, right and left becomes null in case they do not exists</returns>
        public KeyValuePair<T, T> DirectRightAndLeft(T NewItem)
        {
            if (Count == 0)
            {
                return new KeyValuePair<T, T>(default(T), default(T));
            }
        ...
        /// <param name="NewItem">Item to get its left and right nodes</param>
        /// <returns>
        /// KeyValuepair where the key is the right node and value is the left node
        /// Right and left are found in a rotational way i.e. if right is at index Count, then the item at 0 is returned in the key
        /// </returns>
        public KeyValuePair<T, T> DirectRightAndLeftRotational(T NewItem)
        {
            if (Count == 0)
            {
                return new KeyValuePair<T, T>(default(T), default(T));
